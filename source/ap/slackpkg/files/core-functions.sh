...
function get_gpg_key() {
	if ping -c 1 www.slackware.com &>/dev/null; then
		if printf "$ARCH" | grep -q -e "arm" -e "aarch64" -e "loong64" &>/dev/null ; then
			echo -e "\t\t\tGetting key from https://slackware.uk/slackwareloong/slackwareloong64-current/GPG-KEY"
			$DOWNLOADER $TMPDIR/gpgkey https://slackware.uk/slackwareloong/slackwareloong64-current/GPG-KEY &>/dev/null
